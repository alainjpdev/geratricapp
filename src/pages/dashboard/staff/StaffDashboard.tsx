...
export const StaffDashboard: React.FC = () => {
  const [activeTab, setActiveTab] = useState<TabType>(() => {
    ...
    return (params.get('tab') as TabType) || 'staff';
  });
  // Trigger re-compile
  ...
  const canManage = currentUser?.role === 'admin' || currentUser?.role === 'enfermero';
  ...
  const handleDeleteUser = async () => {
    ...
    if (window.confirm('¿Estás seguro de que deseas eliminar este usuario?')) {
      try {
        console.log('🔘 Delete button clicked for user:', selectedUser.email);
        setUpdatingUser(true);
        await deleteUser(selectedUser.id);
        console.log('✅ deleteUser service call completed');
        setToast({ visible: true, message: 'Usuario eliminado', type: 'success' });
        setShowEditUser(false);
        fetchStaff();
      } catch (error) {
        console.error('Error deleting user:', error);
        setToast({ visible: true, message: 'Error al eliminar usuario', type: 'error' });
      } finally {
        setUpdatingUser(false);
      }
    }
