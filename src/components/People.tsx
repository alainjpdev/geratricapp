...
import { UserPlus, Mail, MoreVertical, User } from 'lucide-react';
...
export const People: React.FC<PeopleProps> = ({ classId, isTeacher = true }) => {
  ...
  const loadPeople = async () => {
    ...
    try {
      // TODO: Load from API
      // const res = await apiClient.get(`/api/classes/${classId}/people`);
      ...
      setTeachers([
        {
          ...
          firstName: 'Jean Paul',
          lastName: 'Raimond',
          email: 'ark2784@gmail.com',
          avatar: 'https://lh3.googleusercontent.com/a/ACg8ocLuHx4b80EgFUAwR7EJgbG24V-nH2pQBAgXpQHHPoLW2h5f4e4C_g=s32-c'
        }
      ]);
      setStudents([]);
    } catch (err) {
  ...
  const handleInviteTeachers = () => {
    // TODO: Open invite teachers modal
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {isTeacher && (
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-lg transition-colors hidden"
        ...
        {teachers.length === 0 ? (
          ...
          <ul className="space-y-3">
            {teachers.map((teacher) => (
              <li key={teacher.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition-colors">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        {students.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-lg font-medium text-gray-900 mb-2">Add students to this class</p>
            {isTeacher && (
              <button
                onClick={handleInviteStudents}
          ...
          <ul className="space-y-3">
            {students.map((student) => (
              <li key={student.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition-colors">
                <div className="flex items-center gap-3 flex-1">
                  {student.avatar ? (
                    <img
                      src={student.avatar}
